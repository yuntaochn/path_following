...
泛化实验采用四条路径（L 形单折 90°、Z 形双折 90°、U 形折返、蛇形交替 90°）和三种工况（无扰动、定常海流、含噪海流），每个场景对比 ILOS-PID、ILOS+FO 和 SHCS。四条路径从单次直角转弯递进至多次连续或交替折弯，用于检验同一组 SHCS 参数在不同折弯密度和折弯方向组合下是否保持一致的精度--能耗折中特性。需要强调的是，本节并不以 SHCS 在 CTE-RMS 上全面优于 ILOS-PID 为判断标准；短前视 ILOS-PID 本身具有更激进的几何跟踪能力，SHCS 的目标是在保持可接受路径误差的同时降低偏航能耗和执行器负荷。图~\ref{fig:generalization_traj} 给出无扰动与定常海流下的轨迹对比，图~\ref{fig:generalization_metrics} 给出相同场景下的 CTE-RMS 与偏航能耗柱状图，表~\ref{tab:generalization} 汇总全部 12 个场景中 SHCS 相对 ILOS-PID 的 CTE 代价和偏航能耗收益。

\end{multicols}

\begin{center}
\includegraphics[width=0.98\textwidth]{fig_traj_composite.png}\\
\figcaption{多路径泛化实验轨迹对比（无扰动与定常海流）}
\label{fig:generalization_traj}
\end{center}

\begin{center}
\includegraphics[width=0.98\textwidth]{fig_metrics.png}\\
\figcaption{多路径泛化实验指标对比（无扰动与定常海流）}
...
图~\ref{fig:generalization_traj} 表明，在无扰动和定常海流场景下，ILOS-PID 与 SHCS 的轨迹大体重合，未出现发散、绕行或终点不可达现象；这说明 SHCS 的参考整形并未破坏不同路径拓扑下的基本跟踪能力。相比之下，ILOS+FO 在各路径转弯后均存在更明显的外偏，尤其在 Z 形、U 形和蛇形路径中更为突出，说明固定一阶参考模型虽然能平滑航向命令，却难以兼顾多折线路径的几何跟踪。

定量结果进一步说明，SHCS 的泛化优势主要体现在折中规律的一致性，而不是 CTE-RMS 的单指标优势。由表~\ref{tab:generalization} 可见，SHCS 在全部 12 个场景中均进入 3 m 目标容差圆；无扰动工况下 CTE 代价为 $-0.2\%$--$+1.8\%$，几乎与 ILOS-PID 持平，同时偏航能耗降低 21.0\%--23.3\%；定常海流工况下 CTE 代价为 2.8\%--10.4\%，偏航能耗降低 20.1\%--24.6\%；含噪海流工况下 CTE 代价增大至 13.7\%--19.4\%，但偏航能耗降低也提高至 29.7\%--31.8\%。图~\ref{fig:generalization_metrics} 以无扰动和定常海流为例展示了这种关系：SHCS 的 CTE 柱与 ILOS-PID 接近并显著低于 FO，而偏航能耗在所有路径上均低于 ILOS-PID 和 FO。因此，多路径泛化实验支持的结论是：同一组 SHCS 参数能够在不同折弯密度、折弯方向和扰动强度下保持可接受 CTE，并稳定换取偏航能耗下降，而非在所有路径上获得低于 ILOS-PID 的横向误差。
...
2）基于 CyberShip II 模型的仿真结果表明，在统一前视距离（$\Delta=4$ m）条件下，与 ILOS-PID 相比，SHCS 典型场景下 CTE-RMS 仅增大 6.7\%，同时偏航控制能耗降低 23.2\% 并消除饱和；固定参数整形方法（FR/FO）虽同样消除饱和，但 CTE-RMS 分别恶化 302\% 和 238\%，说明 SHCS 能以接近基线的跟踪精度实现饱和抑制和能耗节省。12 个泛化场景中偏航能耗改善 20.1\%--31.8\%，CTE-RMS 在无扰动和定常海流工况下变化较小、在含噪海流工况下增大约 15\%；随机化蒙特卡洛实验进一步确认精度--执行器负荷折中（CTE +19.6\%、偏航能耗 $-$27.8\%、饱和时间 $-$95.1\%）。
