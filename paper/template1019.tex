...
\section{问题建模}
\subsection{无人听运动学与动力学模型}
本文研究三自由度欠驱动无人艇在水平面内的路径跟踪问题。建立惯性坐标系${n}$和艇体坐标系${b}$。
惯性坐标系用于描述无人艇的位置和艏向，艇体坐标系原点位于艇体质心处，$x_b$轴指向船艏，$y_b$轴指向右舷。

定义无人艇惯性坐标系位姿向量$\eta=[x,y,\psi]^T$，其中$(x,y)$为位置，$\psi \in \pi$,$\psi$为艏向角；
船体坐标系速度向量$\nu = [u, v, r]^T$，其中$u$为纵向速度，$v$为横向速度，$r = \dot{\psi}$为偏航角速度。
...
其中$M$为惯量矩阵（含附加质量），$C(\nu)$为科氏/向心力矩阵，$D(\nu)$为水动力阻尼矩阵，$\tau=[\tau_u,0,tau_r]^T$为控制力向量。由于缺少横向推进器（欠驱动），横向控制力分量为零。

设左、右推进器的推力分别为TL，TR，推进器间距为b，则推力与广义力之间满足

当推进器推力受限于$|T_L|,|T_R|\leq T_{\max}$时，纵荡推力与偏航力矩满足
式（5）表明，偏航力矩需求越大，可用于纵荡推进的余量越少[9]。如果由于航段切换导致较大的偏航需求，则纵荡推进和偏航控制都会在同一双桨可行域内竞争执行器资源。因此，在航点路径跟踪中的执行器受限问题不仅是偏航通道饱和的问题，还包括偏航控制占用纵荡推进余量的问题。

\subsection{LOS导引与航段切换}
LOS根据无人艇当前位置与路径前视点的几何关系生成期望航迹角。其原理如图1所示。
