...
4）构建统一配置下的典型场景、多路径泛化、随机化配对蒙特卡洛、参数敏感性和消融实验协议，对所提方法的误差收益、能耗收益和饱和抑制作用进行验证。
...
\end{multicols}
\begin{center}
	\includegraphics[width=0.9\textwidth]{控制架构图.png}\\
...
\end{equation}
$|e_s|$ 越大，说明当前几何航向命令与可执行参考之间的差异越大，系统通常处于较强转弯或整形尚未完成的阶段。此时若仍维持较高纵荡速度，横向偏离和推进器余量竞争均可能加剧。为此，首先构造基于整形残差的simple模式速度调度律：
\begin{equation}
...
为进一步体现推进器联合约束，coupled 模式通过 PID 的只读 preview 接口估计本步偏航力矩需求 $\tau_r^{pre}$，但不更新 PID 的积分和微分内部状态。由式（\ref{eq:joint_constraint}）可得当前可用纵荡推力上界
\begin{equation}
\tau_{u,\mathrm{avail}}
...
\label{eq:ud_final}
\end{equation}
并对 $u_d$ 进行一阶平滑和斜率限制，以避免速度命令突变。需要指出的是，整形残差项反映导引参考与可执行参考之间的动态不一致，推进器约束项反映偏航力矩对纵荡推力余量的占用，两者分别从参考层和执行器层刻画转弯压力。典型单转弯工况下 coupled 调度与 simple 调度数值接近，说明整形残差信号已足以触发主要降速动作；coupled 项则为更强扰动或连续急转场景提供推进器约束感知能力。
...
本文所有对比实验均采用统一前视距离 $\Delta=4$ m，包括 ILOS-PID 基线。该设置使方法间差异完全来自参考整形和速度调度机制，而非前视距离选择。本文设置 3 类工况：无扰动 calm、定常海流 steady\_current（海流速度为 $[0.35,0.18]$ m/s）和含噪海流 current（在定常海流基础上加入力、控制、位姿和速度噪声）。典型场景、参数敏感性分析和消融实验采用 L 形路径与定常海流组合；泛化实验采用 L 形、Z 形、U 形和蛇形路径，并在上述 3 类工况下验证；蒙特卡洛实验采用 L 形路径，围绕含噪海流构造 30 个随机化配对工况。
...
\figcaption{典型 L 形路径四方法对比（所有方法 $\Delta=4$ m）。图中依次展示轨迹、LOS航向误差、横向误差、原始偏航力矩、纵荡速度和推进器纵荡余量}
\label{fig:typical}
\end{center}
...
泛化实验采用四条路径（L 形单折 90°、Z 形双折 90°、U 形折返、蛇形交替 90°）和三种工况（无扰动、定常海流、含噪海流），每个场景对比 ILOS-PID、ILOS+FO 和 SHCS。四条路径从单次直角转弯递进至多次连续或交替折弯，系统地考察 SHCS 参考整形机制在不同折弯密度和方向组合下的泛化能力。图~\ref{fig:generalization_traj} 展示无扰动与定常海流共 8 个场景的轨迹，表~\ref{tab:generalization} 给出全部 12 个场景的 CTE-RMS 绝对值及 SHCS 相对 ILOS-PID 的改善幅度。

\end{multicols}

\begin{center}
\includegraphics[width=0.98\textwidth]{fig_traj_composite.png}\\
\figcaption{多路径泛化实验轨迹对比（无扰动与定常海流工况）}
...
\begin{center}
\tabcaption{多路径泛化场景下的 CTE-RMS（12 个场景，所有方法均取 $\Delta=4$ m）}
\label{tab:generalization}
\journaltable
\setlength{\tabcolsep}{2.5pt}
\begin{tabular}{@{}llCCC@{}}
\hline
工况 & 路径 & ILOS-PID/m & SHCS/m & CTE变化 \\
\hline
\multirow{4}{*}{无扰动}
  & L形（单折）& 0.400 & 0.402 & $-$0.6\% \\
  & Z形（双折）& 0.463 & 0.462 & $+$0.2\% \\
  & U形折返    & 0.388 & 0.395 & $-$1.8\% \\
  & 蛇形（交替）& 0.471 & 0.476 & $-$1.1\% \\
\hline
\multirow{4}{*}{定常海流}
  & L形（单折）& 0.848 & 0.899 & $-$5.9\% \\
  & Z形（双折）& 0.782 & 0.863 & $-$10.4\% \\
  & U形折返    & 0.714 & 0.734 & $-$2.8\% \\
  & 蛇形（交替）& 0.831 & 0.874 & $-$5.2\% \\
\hline
\multirow{4}{*}{含噪海流}
  & L形（单折）& 0.860 & 1.017 & $-$18.2\% \\
  & Z形（双折）& 0.808 & 0.964 & $-$19.4\% \\
  & U形折返    & 0.712 & 0.809 & $-$13.7\% \\
  & 蛇形（交替）& 0.850 & 0.978 & $-$15.1\% \\
\hline
\end{tabular}

\vspace{2pt}
\parbox{0.96\linewidth}{\footnotesize
注：CTE 变化正值表示 SHCS 改善，负值表示 SHCS 略差；偏航能耗改善见正文，全部 12 个场景均为 20.1\%--31.8\%（SHCS 优于基线）。
}
\end{center}

表~\ref{tab:generalization} 表明，SHCS 在全部 12 个场景中均进入 3 m 目标容差圆，并以一致的偏航能耗节省（20.1\%--31.8\%）为代价换取不同工况下不同程度的 CTE 变化。图~\ref{fig:generalization_traj} 的视觉对比直接佐证了这一趋势：无扰动行的三方法轨迹高度重合，说明 SHCS 在无扰动场景下 CTE 与 ILOS-PID 基本持平（$-$1.8\%\,--\,$+$0.2\%）；定常海流行中 ILOS+FO 在各路径转弯处偏出最远，SHCS 略慢于 ILOS-PID 收敛（CTE 增大 2.8\%--10.4\%）但远优于 FO。定量而言，无扰动工况下 CTE 变化为 $-$1.8\%\,--\,$+$0.2\%，偏航能耗改善 21.0\%--23.3\%；定常海流工况下 CTE 增大 2.8\%--10.4\%，偏航能耗改善 20.1\%--24.6\%；含噪海流工况 CTE 增大 13.7\%--19.4\%，偏航能耗改善 29.7\%--31.8\%（含噪工况下偏航通道因随机扰动额外作功，使节能效果反而更显著）。该工况下的系统性统计规律已由蒙特卡洛实验（\ref{subsec:mc} 节）进一步验证。从路径类型看，U 形折返路径包含三次连续同向折弯，SHCS 的整形参数在各折弯处一致生效，CTE 代价最小（无扰 $-$1.8\%，定常海流 $-$2.8\%）；蛇形路径方向交替，整形残差信号正负交替自适应调整参考变化率，说明同一组 SHCS 参数能够迁移到不同折弯方向组合的路径。ILOS+FO 在全部 12 个场景中的 CTE-RMS 均显著高于 ILOS-PID（无扰动 163\%，定常海流 230\%），说明固定时间常数滤波以巨大的精度代价换取饱和抑制，而 SHCS 能以接近基线的 CTE 代价达到同等饱和抑制效果。
...
\begin{center}
\tabcaption{随机化蒙特卡洛统计结果（$N=30$ 配对工况，L 形路径 + 含噪海流）}
\label{tab:mc}
\journaltable
...
\vspace{2pt}
\parbox{0.96\linewidth}{\footnotesize
注：数据为均值$\pm$95\%\,CI；配对符号秩检验，各指标均满足 $p<0.001$。偏航能耗和饱和时间在全部 30 个工况下 SHCS 均优于基线（30/30），CTE-RMS 在全部工况下 SHCS 均高于基线（0/30），详见图~\ref{fig:mc_summary}。
}
\end{center}

\begin{center}
\includegraphics[width=\columnwidth]{fig4_mc_paired_col.png}\\
\figcaption{随机化蒙特卡洛 30 个配对工况斜率图。每条细线连接同一工况下 ILOS-PID（$\Delta=4$ m）与 SHCS 的指标值，蓝色线表示该工况 SHCS 改善，橙色线表示 SHCS 变差；菱形及误差棒表示均值和 95\% 置信区间，粗线表示均值趋势；右上角数值为 SHCS 相对 ILOS-PID 的均值变化百分比及配对检验 $p$ 值}
\label{fig:mc_summary}
\end{center}

表~\ref{tab:mc} 和图~\ref{fig:mc_summary} 表明，在同前视距离条件下，SHCS 并非在所有指标上优于 ILOS-PID。CTE-RMS 均值由 0.8870 m 增至 1.0606 m，约为 19.56\% 的相对代价，且 30 个随机工况中均呈现这一趋势，说明短前视 ILOS-PID 的几何跟踪更激进。几何 LOS 航向误差（$\psi_d - \psi$）方面，SHCS 均值略升至 0.16360 rad，相较基线 0.14901 rad 增加 9.79\%，仅 6/30 个工况中 SHCS 的 LOS 航向误差更低；这表明参考整形在 LOS 几何层面引入了轻微滞后，并非在 LOS 航向跟踪精度上胜过激进跟踪的 ILOS-PID。与此同时，SHCS 将偏航能耗从 807.17 降至 582.91，降幅 27.78\%，30 个工况中均保持下降；将原始偏航饱和时间从 0.707 s 降至 0.035 s，降幅 95.05\%，30 个工况中均保持下降。因此，随机化蒙特卡洛实验支持”精度--执行器负荷折中”的结论：参考整形层以较大横向误差代价和轻微 LOS 航向精度损失，换取显著的偏航能耗节省（27.8\%）和饱和抑制效果（95.1\%）。
...
首先考察降速强度 $\lambda$ 的影响。当 $\lambda$ 从 0.1 增大至 1.0 时，横向误差均方根值仅发生小幅变化，整体变化幅度小于 0.005 m。这说明在当前测试场景下，航向整形已承担主要参考可执行化作用，速度调度强度对路径跟踪精度的边际影响相对有限。同时，偏航能耗随 $\lambda$ 增大略有上升，表明过强降速可能改变转弯过程中的速度分布，使控制过程不一定获得更低偏航能耗。综合横向误差和控制能耗两项指标，本文取 $\lambda=0.6$ 作为默认值。

其次考察名义偏航角速度上限 $r_{\mathrm{nom}}$ 的影响。当 $r_{\mathrm{nom}}$ 取值较小时，航向参考整形过程过于保守，无人艇在转弯过程中难以及时跟随期望路径，导致横向误差明显增大。当 $r_{\mathrm{nom}}$ 位于 0.7--1.5 rad/s 区间时，横向误差保持在较低水平，说明该范围内的整形速率能够较好兼顾路径跟踪精度和参考可执行性。当 $r_{\mathrm{nom}}$ 进一步增大时，横向误差变化不明显，但偏航能耗有所增加，说明过大的整形速率会削弱航向整形对控制能耗和执行器饱和风险的抑制作用。
...
由上述分析可知，降速强度 $\lambda$ 对整体性能影响较为平缓，而名义偏航角速度上限 $r_{\mathrm{nom}}$ 对航向整形效果更为敏感。在实际应用中，$r_{\mathrm{nom}}$ 应结合目标 USV 的偏航动力学特性和推进器能力进行整定。本文采用的 $r_{\mathrm{nom}}=1.5$ rad/s 能够在路径跟踪精度、偏航能耗和推进器约束之间取得较好折中。


\subsection{消融实验}
为进一步分离抗积分卷绕、动态整形和速度调度的贡献，本文在 L 形路径和定常海流条件下开展消融实验。所有方法统一使用 $\Delta=4$ m 前视距离，消融序列为：ILOS（$\Delta=4$ m 基线）$\to$ AW（加入反算抗积分卷绕）$\to$ DS（加入动态航向整形）$\to$ SHCS（加入速度调度，完整方法）。饱和时间根据分配前的原始偏航控制输出统计，当其绝对值超过 $0.97\tau_{r,\max}$ 时认为发生饱和。实验结果如图~\ref{fig:ablation} 所示。

\begin{center}
\includegraphics[width=\columnwidth]{fig6f_bar.png}\\
\figcaption{消融实验关键指标对比（L 形路径，定常海流，所有方法 $\Delta=4$ m）。各方法按消融步骤从左到右递进：ILOS$\to$AW$\to$DS$\to$SHCS，SHCS 以砖红色与粗边突出}
\label{fig:ablation}
\end{center}

图~\ref{fig:ablation} 展示了各组件的边际贡献（消融链起点均使用 $\Delta=4$ m）。第一步加入反算抗积分卷绕（AW）后，饱和时间由 0.65~s 降至 0.55~s，但 CTE-RMS 从 0.857~m 升至 1.074~m（增大 25\%），偏航能耗从 103.1 升至 123.8（增大 20\%），表明在短前视条件下 AW 虽减少积分饱和累积，但其对 PID 积分状态的回算处理改变了控制响应，并未从根本上解决航段切换处的参考不可执行问题。第二步加入动态航向整形（DS）后，CTE-RMS 从 1.074~m 降至 0.909~m，偏航能耗从 123.8 降至 75.2，饱和时间完全消除，LOS 航向误差 RMS 从 0.1346~rad 降至 0.0454~rad（改善 66\%），说明动态航向整形是饱和抑制和能耗节省的核心机制。第三步加入完整 SHCS 后，CTE-RMS 和航向误差相对 DS 变化极小（0.909 vs 0.914~m），偏航能耗从 75.2 略升至 79.2，但总能耗由 1932.6 降至 1793.7，表明速度调度层主要改善推进器余量与纵荡能耗分配，对偏航通道指标影响有限。推进器约束感知调度在典型单转弯场景中的边际作用不显著，其效果更适合在更强扰动或连续急转场景中进一步检验。
...
2）基于 CyberShip II 模型的仿真结果表明，在统一前视距离（$\Delta=4$ m）条件下，与 ILOS-PID 相比，SHCS 典型场景下 CTE-RMS 仅增大 6.7\%，同时偏航控制能耗降低 23.2\% 并消除饱和；固定参数整形方法（FR/FO）虽同样消除饱和，但 CTE-RMS 分别恶化 302\% 和 238\%，说明 SHCS 能以接近基线的跟踪精度实现饱和抑制和能耗节省。12 个泛化场景中偏航能耗改善 20.1\%--31.8\%，CTE-RMS 在低噪工况下基本持平、在含噪工况下增大约 15\%；随机化蒙特卡洛实验进一步确认精度--执行器负荷折中（CTE +19.6\%、偏航能耗 $-$27.8\%、饱和时间 $-$95.1\%）。

3）与直接改进 LOS 导引律、固定滤波或单独抗积分卷绕方法相比，SHCS 保留原有 ILOS-PID 主体结构，仅在导引与控制之间引入与船体偏航能力和双桨约束相关的参考整形环节。消融实验表明，动态航向整形对误差改善和饱和抑制起主要作用，速度调度对推进余量协调具有辅助作用。

4）本文研究仍以数值仿真为主，对时变海流、波浪、推进器迟滞和传感器误差等实际因素的考虑尚不充分。后续将结合水池实验或实艇试验，进一步检验所提方法在复杂环境和实际推进器约束条件下的适用性。
